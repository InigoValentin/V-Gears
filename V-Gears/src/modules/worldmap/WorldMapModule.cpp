...
namespace VGears {
    ...
    float ToTextureCoordU(uint8 coord, uint16 id){
        return
          (float(coord) - float(textures[id].u)) / float(textures[id].width);
    ...
    float ToTextureCoordV(uint8 coord, uint16 id){
        return
          (float(coord) - float(textures[id].v)) / float(textures[id].height);
        ...
        if (loaded_textures.empty()){
            for (int i = 0; i < sizeof(textures) / sizeof(textures[0]); i ++){
                std::string texture_name
                  = std::string(textures[i].name) + ".tex";
                ...
                material->getTechnique(0)->getPass(0)->setTextureFiltering(
                  TFO_NONE
                );
