...
void XmlScreenFile::LoadScreen(){
    ...
    if (script != "") ScriptManager::getSingleton().RunFile(script);
    UiWidget* widget = new UiWidget(base_name);
    widget->SetVisible(GetBool(node, "visible", false));
    node = node->FirstChild();
    if (node != nullptr) LoadScreenRecursive(node, base_name, widget);
    UiManager::getSingleton().AddWidget(widget);
}

void XmlScreenFile::LoadScreenRecursive(
            ...
            if (node->ValueStr() == "prototype"){
                ...
                if (name != ""){
                    ...
                    if (origin_x_str != ""){
                        float x_percent = 0;
                        float x = 0;
                        ParsePercent(x_percent, x, origin_x_str);
                    ...
                    if (origin_y_str != ""){
                        float y_percent = 0;
                        float y = 0;
                        ParsePercent(y_percent, y, origin_y_str);
                    ...
                    if (x_str != ""){
                        float x_percent = 0;
                        float x = 0;
                        ParsePercent(x_percent, x, x_str);
                    ...
                    if (scissor_area != ""){
                        ...
                        if (coords.size() == 4){
                            ...
                            ParsePercent(percent_x1, x1, coords[0]);
                            ParsePercent(percent_y1, y1, coords[1]);
                ...
                if (x != ""){
                    ...
                    for (unsigned int i = 0; i < key_frame.size(); ++ i){
                        ...
                        if (data.size() > 1){
                            UiKeyFrameVector2 key;
                            key.time = ParseKeyFrameTime(anim_length, data[0]);
                            ParsePercent(key.value.x, key.value.y, data[1]);
