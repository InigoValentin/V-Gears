...
ParticleSystemManager::ParticleSystemManager(){
    ...
    Ogre::ScriptCompilerManager::getSingleton().addTranslatorManager(translator_manager_);
    Ogre::ScriptCompilerManager::getSingleton().addScriptPattern("*.effects");
    particle_system_factory_ = new ParticleSystemFactory();
    Ogre::Root::getSingleton().addMovableObjectFactory(particle_system_factory_);
    AddEmitterFactory(new PointEmitterFactory());
    AddRendererFactory(new ParticleEntityRendererFactory());
}

ParticleSystemManager::~ParticleSystemManager(){
    Ogre::ScriptCompilerManager::getSingleton().removeTranslatorManager(translator_manager_);
    delete translator_manager_;
    if (particle_system_factory_){
        Ogre::Root::getSingleton().removeMovableObjectFactory(particle_system_factory_);
    ...
    for (t = particle_system_templates_.begin(); t != particle_system_templates_.end(); ++ t)
        delete t->second;
    particle_system_templates_.clear();
    EmitterFactoryMap::iterator efi;
    for (efi = emitter_factories_.begin(); efi != emitter_factories_.end(); ++ efi)
        delete efi->second;
    RendererFactoryMap::iterator rfi;
    for (rfi = renderer_factories_.begin(); rfi != renderer_factories_.end(); ++ rfi)
        delete rfi->second;
}

ParticleSystem* ParticleSystemManager::CreateParticleSystemTemplate(const Ogre::String& name){
    // Validate name.
    if (particle_system_templates_.find(name) != particle_system_templates_.end()){
        OGRE_EXCEPT(
...
ParticleSystem* ParticleSystemManager::GetParticleSystemTemplate(const Ogre::String& template_name){
    ParticleSystemTemplateMap::iterator i = particle_system_templates_.find(template_name);
...
void ParticleSystemManager::ParticleSystemTemplateNames(std::vector<Ogre::String>& v){
    ...
    ParticleSystemTemplateMap::iterator it_end = particle_system_templates_.end();
    for (it = particle_system_templates_.begin(); it != it_end; ++ it) v.push_back((*it).first);
}
...
void ParticleSystemManager::DestroyEmitter(ParticleEmitter* emitter){
    // Locate emitter type.
    EmitterFactoryMap::iterator it = emitter_factories_.find(emitter->GetEmitterType());
    if (it == emitter_factories_.end()){
        OGRE_EXCEPT(
          Ogre::Exception::ERR_INVALIDPARAMS,
          "Particle System: Cannot find requested emitter type.",
...
void ParticleSystemManager::DestroyRenderer(ParticleRenderer* renderer){
    // Locate renderer type
    RendererFactoryMap::iterator it = renderer_factories_.find(renderer->GetRendererType());
    if (it == renderer_factories_.end()){
        OGRE_EXCEPT(
          Ogre::Exception::ERR_INVALIDPARAMS,
          "Renderer: Cannot find requested renderer type.",
