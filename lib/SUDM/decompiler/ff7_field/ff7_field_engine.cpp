...
void FF7::FF7ModuleInstruction::processBATTLE(CodeGenerator* codeGen)
{
    ...
    codeGen->addOutputLine((boost::format("entity_manager:battle_run(%1%)") % battleId).str());
}

void FF7::FF7ModuleInstruction::processBTLON(CodeGenerator* codeGen)
{
    codeGen->addOutputLine((boost::format("entity_manager:random_encounters_on(%1%)") % FF7CodeGeneratorHelpers::FormatInvertedBool(_params[0]->getUnsigned())).str());
}
