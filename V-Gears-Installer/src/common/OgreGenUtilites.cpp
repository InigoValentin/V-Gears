...
void CreateTextureFromVram(
  const Ogre::PixelBox& pixel_box, Vram* vram,
  const int start_x, const int start_y, const int clut_x, const int clut_y,
  const int texture_x, const int texture_y, const BPP bpp,
            ...
            if (bpp == BPP_4){
                ...
                u32 ogre_pixel1
                  = (((clut1 & 0x1f) * 255 / 31) << 0x18)
                    | ((((clut1 >>  5) & 0x1f) * 255 / 31) << 0x10)
                    | ((((clut1 >> 10) & 0x1f) * 255 / 31) << 0x8)
                    | 0;
                u8 stp = (clut1 & 0x8000) >> 15;
                AddTransparency(ogre_pixel1, transparency, stp > 0);
                u32 ogre_pixel2
                  = (((clut2 & 0x1f) * 255 / 31) << 0x18)
                    | ((((clut2 >>  5) & 0x1f) * 255 / 31) << 0x10)
                    | ((((clut2 >> 10) & 0x1f) * 255 / 31) << 0x8)
                    | 0;
                stp = (clut2 & 0x8000) >> 15;
                ...
                u32 ogre_pixel
                  = (((clut & 0x1f) * 255 / 31) << 0x18)
                    | ((((clut >>  5) & 0x1f) * 255 / 31) << 0x10)
                    | ((((clut >> 10) & 0x1f) * 255 / 31) << 0x8)
                    | 0;
                u8 stp = (clut & 0x8000) >> 15;
                AddTransparency(ogre_pixel, transparency, stp > 0);
                data[start_x + x ] = ogre_pixel;
            }
            else if (bpp == BPP_BLACK) data[ start_x + x ] = 0xffffffff;
        }
    }
}

void CreateTexture(
  Vram* vram, const MeshData& mesh_data, const Ogre::String& texture_file_name,
  const VectorTexForGen& textures
    ...
    for (unsigned int i = 0; i < textures.size(); ++ i){
        ...
        CreateTextureFromVram(
          pixel_box, vram, textures[i].start_x, textures[i].start_y,
          textures[i].palette_x, textures[i].palette_y,
          textures[i].texture_x, textures[i].texture_y, textures[i].bpp, true
        );
    }
    Ogre::Image image;
...
void AddTransparency(u32& colour, const bool transparency, const bool stp){
    if (transparency == false){
        if (stp == false && (colour & 0xffffff00) == 0) colour |= 0;
        else colour |= 255;
    }
    else{
        if (stp == false && (colour & 0xffffff00) == 0) colour |= 0;
        else if (stp == false && (colour & 0xffffff00) != 0) colour |= 255;
        else if (stp == true && (colour & 0xffffff00) == 0) colour |= 255;
        else if (stp == true && (colour & 0xffffff00) != 0) colour |= 255 / 4;
    }
}
