...
main( int argc, char *argv[] )
{
    ...
    std::cout << "[]OGRE INITIALIZED" << std::endl;
    fill_names();
    std::cout << "[]NAMES FILLED" << std::endl;
    ...
    f.name = "4sbwy_1"; // DO NOT put a full path here, else it will attempt to save with full output path and fail
    fields.push_back(f);
    ...
    /*for (int f = 0; f < fields.size(); ++f)
    {
        ...
        dat.DumpScript("export_jp/" + fields[f].name + "_script.txt");
        dat.DumpText("export_jp/" + fields[f].name + "_text.txt", false);
        dat.DumpWalkmesh("export_jp/" + fields[f].name + "_wm.xml");
    }
    for (int f = 0; f < fields.size(); ++f)
    {
        DatFile dat("data/jp_in/" + fields[f].name + ".dat");
        dat.DumpScript("export_jp_in/" + fields[f].name + "_script.txt");
        dat.DumpText("export_jp_in/" + fields[f].name + "_text.txt", false);
        dat.DumpWalkmesh("export_jp_in/" + fields[f].name + "_wm.xml");
    }*/
