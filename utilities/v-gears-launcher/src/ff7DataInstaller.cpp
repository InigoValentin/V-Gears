...
int FF7DataInstaller::Progress()
{
    ...
    case eIdle:
        ...
        mTexturesLgp = std::make_unique<ScopedLgp>(mApp.getRoot(), mInputDir + "field/flevel.lgp", "TEX", "FFVIITextures");
        // TODO: DEBUG
...
static void exportMesh(std::string outdir, const Ogre::MeshPtr &mesh)
{
    ...
    while (it.hasMoreElements())
    {
        ...
        if (mat != nullptr)
        {
                            ...
                            for (size_t textureUnitNum = 0; textureUnitNum < pass->getNumTextureUnitStates(); textureUnitNum++)
                            {
                                ...
                                if (unit)
                                {
                                    if (unit->getTextureName().empty() == false)
                                    {
                                        ...
                                        std::cout << "[TEXTURE] " << unit->getTextureName() << " | " << (FieldModelDir() + "/" + baseMeshName + "_" + baseName + ".png") << std::endl;
                                    }
                                }
                            }
                        }
            ...
            if (std::count(writtenMaterials.begin(), writtenMaterials.end(), sub_mesh->getMaterialName()) == 0){
                //std::cout << "[MATERIAL] Writting material " << sub_mesh->getMaterialName() << std::endl;
                mat_ser.queueForExport(mat);
                writtenMaterials.push_back(sub_mesh->getMaterialName());
            }
            //else{
                //std::cout << "[MATERIAL] Not rewritting material " << sub_mesh->getMaterialName() << std::endl;
            //}
        }
        ++i;
    }
    ...
    {
        //std::cout << "    [EXPORTMESH] Texture " << textureName.c_str() << std::endl;
        std::string texName = textureName.c_str();
        std::cout << "[EXPORTMESH] Texture name " << texName << std::endl;
        try{
            Ogre::TexturePtr texturePtr = Ogre::TextureManager::getSingleton().load(texName, "FFVIITextures");//"FFVII");
            //Ogre::TexturePtr texturePtr = Ogre::TextureManager::getSingleton().create(texName, "FFVIITextures");//"FFVII");
            ...
            std::cout << "    [EXPORTMESH] name split: " << baseName << std::endl;
            std::cout << "    [EXPORTMESH] saving: " << (outdir + baseMeshName + "_" + baseName + ".png") << std::endl;
...
public:
    ...
    virtual std::string AnimationName(int charId, int id) override
    {
        ...
        {
            std::cout << "[ERROR] In field " << mFieldName << " the model " << modelInfo.name << " animation with ID " << id  << " is out of bounds (" << modelInfo.animations.size() << ")" << std::endl;
            return std::to_string(id);
        }
    ...
    try
    {
        ...
        if (scriptFile.is_open())
        {
            ...
            try{
                ...
                fnWriteOutputLine("[ERROR] Failed to read texts from field " + field->getName() + ": " + ex.what());
                std::cerr << "[ERROR] Failed to read texts from field " << field->getName() << ": " << ex.what() << std::endl;
            }
        }
        else{
            fnWriteOutputLine("[ERROR] Failed to open script file for writing");
            std::cerr << "[ERROR] Failed to open script file for writing" << std::endl;
        }
    }
    catch (const ::InternalDecompilerError& ex)
    { 
        fnWriteOutputLine("[ERROR] Internal decompiler error in field " + field->getName());
        std::cerr << "[ERROR] Internal decompiler error in field " << field->getName() << ": " << ex.what() << std::endl;
    }
    ...
    // Write out the map file which links all the other files together for a given field
    {
        ...
        if (spawnIterator != std::end(spawnMap))
        {
            ...
            for (size_t i = 0; i < spawnPointRecords.size(); i++)
            {
                ...
                {
                    std::cout << "[WARNING] In field " << field->getName() << ": Map jump triange (" << triIndex << ") out of bounds (" <<field->getWalkmesh()->getTriangles().size() << ")" << std::endl;
                    triIndex = 0;
                }
        ...
        bgImage->encode("png");
        bgImage->save(outDir + "/" + FieldMapDir() + "/" + field->getName() + "/tiles.png");//"/tiles.png");
...
/**
 * Convert the fields from PC to Q-Gears format.
 */
void FF7DataInstaller::ConvertFieldsIteration(){
    ...
    if (mIteratorCounter < mFLevelFileList->size()){
        ...
        if (IsAFieldFile(resourceName)){
            ...
            if (/*IsTestField(resourceName) &&*/!WillCrash(resourceName)){
                //mfnWriteOutputLine("Converting field " + resourceName);
                //std::cout << " - Converting field: " << resourceName << std::endl;
                ...
                std::cout << "[ERROR] Skip field: " << resourceName << " as it has a crash or hang issue" << std::endl;
            }
        }
        mIteratorCounter++;
    }
...
/**
 * Sets up variables for ConvertFieldModelsIteration.
 */
    ...
    if (mModelAnimationMapIterator != mModelsAndAnimationsUsedByConvertedFields.mMap.end()){
        if (mConversionStep == 0){
            ...
            try{
                ...
                mfnWriteOutputLine("[ERROR] Ogre exception converting model " + mModelAnimationMapIterator->first + ": " + ex.what());
                std::cout << "[ERROR] Exception converting model " << mModelAnimationMapIterator->first <<": " << ex.what() << std::endl;
            }
        ...
        mfnWriteOutputLine("[INSTALL] Finished");
        mState = eDone;
    }
    //std::cout << "ConvertFieldModelsIteration: " << mIteratorCounter << "/" << mModelsAndAnimationsUsedByConvertedFields.mMap.size() << "\n";
}
