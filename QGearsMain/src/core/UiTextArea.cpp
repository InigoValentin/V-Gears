    ...
    if( m_Visible == true )
    {
        if( m_Timer == true )
        {
            ...
            if( time != m_TimerTime )
            {
                ...
                time_string += Ogre::StringUtil::format("%2d", m_TimerTime / 60);
...
UiTextArea::Render()
{
    if( m_UpdateTransformation == false && m_Visible == true )
    {
        if(m_RenderOp.vertexData->vertexCount != 0)
        {
            ...
            m_RenderSystem->setScissorTest(true, Ogre::Rect(m_ScissorLeft, m_ScissorTop, m_ScissorRight, m_ScissorBottom));
...
UiTextArea::SetFont(const Ogre::String& font)
{
    ...
    m_Material = Ogre::MaterialManager::getSingleton().getByName("UiMaterials." + m_PathName, "General");
